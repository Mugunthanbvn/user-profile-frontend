...
export class LoginService {
  ...
  static setCurrentUserData = (currentUserData: ICurrentUserData) => {
    ...
    const decodedToken: { expiry: number; email: string } = jwtDecode(
    ...
    if (expiryTime > 0)
      LoginService.expiryHandler = setTimeout(
        () => LoginService.logout(),
        expiryTime * 1000,
      );
    else LoginService.expiryHandler = null;
  };
  ...
  static async login(creds: ILoginCreds) {
    ...
    try {
      ...
      if (response.status === 200) {
        ...
        LoginService.setCurrentUserData({
          email: creds.email,
  ...
  static getJwtToken() {
    try {
      ...
      const decodedToken: { expiry: number; email: string } =
        jwtDecode(jwtToken);
      ...
      const currentTokenCheck =
        LoginService.currentUserData &&
        LoginService.currentUserData?.token === jwtToken;
