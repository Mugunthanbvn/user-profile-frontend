...
export const InputField = (props: IInputFieldProps)=>{
    const {title, value= '',type = 'text',subTitle,className, isRequired =  false,placeholder,minLength, maxLength,pattern,validate, onChange, onValueUpdate} = props;
    ...
    const valueUpdateHandler = (e: any )=>{
        if(e.target?.value !=null){
            onValueUpdate?.(e.target.value);
                const validationMsg = e.target.validationMessage || validate?.(e.target.value);
                if(validationMsg){
    ...
    return <div className={classNames({'input-field-container': true, [className]: className})}>
        ...
        <input type={inputType} value={inputValue} required={isRequired}
        ...
        ref={inputRef}
        />
        {type === 'password' && <EyeIcon className={classNames({
            'icon': true,
            'active': inputType==='password'
        })} onClick={(e)=>{e.stopPropagation();setInputType(inputType==='password'? 'text': 'password')}} />}
        </div>
