...
const Login = ()=>{
    ...
    const loginHandler = ()=>{
        ...
        LoginService.login({email, password}).then((status)=> { if(status === ELoginStatus.sucess){
            navigate('/profile')
        }}).catch((e)=>{
            setError(e.toString());
        });
    ...
    const renderLoginContainer = ()=>{
        ...
        <a className="login-forgot-password" onClick={()=>{}}>Forgot password?</a>
        </div>
        {error && <div className="login-error">
            {error}
            </div>}
        <Button label="Login" type={'submit'} onClick={loginHandler}/>
        <div className="login-signup"><span >Don’t have an account ?</span><a href="/register">Sign up</a></div>
    </div>
    }
